...
Этот минус, и многие другие минусы WebTutor, решаются его плюсом, а именно его открытостью. То есть если WebTutor что-то не поддерживает и у вас не получается, что-то реализовать, всегда есть возможность запустить сторонний сервер и использовать его модули для взаимодействия с WebTutor. Я к примеру использую для этого Node.js и npm, можно на свое усмотрение использовать любой сервер и любой язык\(PHP, Go, Python итп\)
