...
Хочется конечно верить, что поддержка IE9, IE10 действительно есть. Но рекомендую вам рассчитывать только на поддержку IE.

P.S. Можете попробовать сделать компоненты через X-Tag, все-таки это разработка Microsoft и они там как-то осуществили поддержку для старых IE. Х
